...
- **스테이지 12개로 확대** (테마당 3개): 1~3 코드 영역(정연한 가로 행), 4~6 데이터 영역(블록 격자), 7~9 스택(위에서 쌓이는 지그재그), 10~12 힙(단편화 조각). 배치는 스펙 §6 표 그대로 (ui: 1~2 button, 3~4 hint, 5~12 free; 개념: 5 변수, 6 조건문, 7 반복문, 8 함수, 9 복합, 10 테이블, 11 종합, 12 최종; 보상 4 cache·6 webhook; 퍼즐 6·9·12). 12×16, 건설칸 ≥ 6. 신규 스테이지(9~12)의 solution/hints/문제 서술/버튼 없음(free)/난이도 규칙(이하 개념만) 포함. 회귀 루프는 1..12로 확장
...
- [ ] **Step 3: timelines.csv 재설계** — **12개 스테이지** (9~12 신규 — stages.csv 12행 확장은 이 태스크에서 함께: 스펙 §6 표의 ui/concept/theme/problem/보상/퍼즐 열 값과 mazes/curriculum 파일 참조를 먼저 채우되, 미로·커리큘럼 파일 실체는 Task 3에서 생성하므로 **이 태스크에서는 기존 8스테이지의 리밸런싱 + validate 규칙까지만 커밋**하고 12개 확장은 Task 3에 위임한다 — 아래 예시는 스테이지 1): 첫 이벤트 at 0~10, 이후 이벤트가 ~280초까지 공백 ≤ 25초로 이어지도록 (규칙 상한 40의 여유 안). 스테이지 1은 초반 저밀도(튜토리얼 읽기), 전 스테이지 후반 밀도 상승. 적 구성은 기존 종류 분포 유지(신규 종류 금지). 예시 형태 (스테이지 1):
...
- [ ] **Step 1: 미로 12종 재설계** — 테마별 형태 (12열×16행, `#`벽/`.`통로/`B`건설칸 ≥ 6, 1행 통로 = 스폰 열, 맨 아랫줄 통로 ≥ 1, 전 통로가 아랫줄 도달 가능). 같은 테마 3개는 같은 문법의 변주(난이도 상승). **6·9·12는 퍼즐 강제 구조** (스펙 §6.5·§6 표: 6 = 특정 위치 컴파일러+스나이퍼 필수 — timeline null-ptr 밀도와 연계 조정 허용, 9 = 유효 사거리 칸 극소수, 12 = 분할 지점 배치 필수):
  - 001~003 코드 영역: 가로 행 통로가 명령어 줄처럼 규칙적으로 반복, 행 사이 벽 랙
  - 004~006 데이터 영역: 4×3 블록 벽들이 격자 배치, 블록 사이 십자 통로
  - 007~009 스택: 좌우 번갈아 뚫린 층계 지그재그, 아래로 갈수록 통로 폭 감소
  - 010~012 힙: 크기가 제각각인 벽 조각을 흩뿌린 단편화 구조 (통로는 유기적 곡류)
- [ ] **Step 2: 12스테이지 데이터 확장 + 좌표 동기화** — stages.csv를 스펙 §6 표대로 12행으로 (theme/problem/ui/concept/보상/puzzle/naive_file/countdown/tutorial_file/buttons_file), timelines.csv 9~12 신규 작성(§2 규칙 준수, null-ptr 6+, concat-nil 8+ 분포 유지·확장), 신규 solution 9~12 + hints 9~12(free — 힌트는 개념 안내 주석 수준) + naive 3종(6·9·12) 작성. 각 스테이지: 새 미로의 건설칸에서 정답 배치 좌표 선정(예산 내) → solution build 좌표 갱신 → buttons_1/2 스크립트 좌표 갱신(스테이지 1: 정답 2좌표와 동일) → tutorial_1.lua 앵커 2셀을 그 좌표로 → timelines 스폰 열을 새 1행 통로로. 예산: 9~12는 300~400 범위에서 설계(스나이퍼/컴파일러 조합 여지).
- [ ] **Step 3: 회귀로 검증** — 12스테이지 정답 클리어 + 퍼즐 3종 naive defeat + validate 0오류 + 전 스위트. 클리어 안 되는 스테이지는 배치 좌표(브루트포스 허용) 또는 timeline 미세 조정.
